...
USING (auth.uid() = user_id OR public.is_creator(auth.uid()));

CREATE POLICY "Only creators can insert roles"
ON public.user_roles FOR INSERT
WITH CHECK (public.is_creator(auth.uid()));

CREATE POLICY "Only creators can update roles"
ON public.user_roles FOR UPDATE
USING (public.is_creator(auth.uid()));

CREATE POLICY "Only creators can delete roles"
